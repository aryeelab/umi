...
"""
def fq_get_bcmolid(read01, read02, umi_len, restriction_site_index, rs_len, molid_len):
    umi = read01[1][:int(umi_len)]
    barcode = read01[1][int(umi_len):int(restriction_site_index)]
    id_pos = int(restriction_site_index) + int(rs_len)
...
"""
def fq_umitag(fastq_file01, fastq_file02, umi_len, barcode_len, restriction_site, molid_len, out_name01, out_name02, out_dir):
    ...
    rs_len = len(restriction_site)

    for item01, item02 in itertools.izip(fq_open(fastq_file01), fq_open(fastq_file02)):
        restriction_site_index = int(umi_len) + int(barcode_len)

        # Restrict to reads with UMI tag and Sample Barcode of full length and with the restriction site as expected
        if item01[1][restriction_site_index:(restriction_site_index + rs_len)] == restriction_site:
            xtag = fq_get_bcmolid(item01, item02, umi_len, restriction_site_index, rs_len, molid_len)
            ...
            item01[0] = '%s %s\n' % (header01, xtag) 
            item01[1] = item01[1][restriction_site_index:]
            item01[3] = item01[3][restriction_site_index:]
            item02[0] = '%s %s\n' % (header02, xtag)
...
def main():
    ...
    parser.add_argument('--restriction_site', help='restriction enzyme site', required=True)
    parser.add_argument('--molid_len', help='length of genenomic sequence following the restriction site that will be used in the tag', default=6)
    ...
    print("Parameters: number of bases in \n barcode = %s \n UMI tag %s \n molecular id = %s" \
          % (args.barcode_len, args.umi_len, args.molid_len), file=sys.stderr)

    fq_umitag(args.fastq_file01, args.fastq_file02, args.umi_len, args.barcode_len, args.restriction_site, args.molid_len, args.out_name01, args.out_name02, args.out_dir)
